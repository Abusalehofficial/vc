<?php
 ...
 if($_REQUEST['err_msg']=="Success"){
  $payment = $conn->prepare('SELECT * FROM payments INNER JOIN clients ON clients.client_id=payments.client_id WHERE payments.payment_extra=:extra ');
                $payment->execute(['extra' => $_REQUEST['basket_id']]);
                $payment = $payment->fetch(PDO::FETCH_ASSOC);
                
               
                
              
                $payment['payment_amount'] = convertCurrencyUpdated("PKR", $settings["site_currency"], $payment['payment_amount']);
