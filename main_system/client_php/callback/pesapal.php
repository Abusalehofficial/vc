<?php
  ...
  $pesapal_notification_type        = $_REQUEST['pesapal_notification_type'];
$pesapal_transaction_tracking_id  = $_REQUEST['pesapal_transaction_tracking_id'];
$pesapal_merchant_reference       = $_REQUEST['pesapal_merchant_reference'];
      ...
      if ($pesapal_merchant_reference) {
        ...
        $payment = $conn->prepare('SELECT * FROM payments 
          ...
          WHERE payments.payment_extra = :extra AND payments.payment_status != 3');
    $payment->execute(['extra' => $pesapal_merchant_reference]);
    $payment = $payment->fetch(PDO::FETCH_ASSOC);

    $payment['payment_amount'] = $payment['payment_amount'] /2600;
    ...
    if ($update && $balance && $insert) {
        ...
        $conn->rollBack();
        // Handle the case where database operations failed
        header("Location: " . site_url('addfunds'));
     }
    }
